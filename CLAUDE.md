...
- **Round-9 polish (Jun 6 PM):** removed the last two AR/VR **golden glows** (bg-accent BG glow + VR-face radial glow) and the `.wave-divider svg` **drop-shadow** (the faint "lines" under the wave); faster, **seamless hero load** — cut the framer entrance delays (subtitle 1.0→0.22s, planner 1.25→0.36s, presence 0.2→0.05s) **and the big one:** the hero globe was a **1.9 MB PNG served raw to every device** (`images.unoptimized:true`). Converted it to a **202 KB WebP** (`ffmpeg -c:v libwebp -quality 90`, 1200² preserved, visually identical) → ~9.4× smaller, kept `unoptimized` so behaviour is identical on every host (no runtime `sharp` dependency). How-it-works **"Start your journey" CTA** no longer clips on short/landscape viewports (laptops, iPad-landscape ≈700px): added `@media (max-height:860/740px)` compression of the sticky stack's vertical rhythm.
...
3. **Preview can't measure `vh`/`svh`/`dvh` layouts on the homepage:** it resolves viewport-height units to **0**, collapsing the hero/How-it-works/etc. so `offsetHeight`/`getBoundingClientRect` of those (and any below-the-fold) sections read **0** even though `getComputedStyle().height` is right. `innerHeight` (JS) stays correct. Real browsers are fine — this is preview-only. For `vh`-heavy layout checks, reason from JS `innerHeight` + computed styles / layout math, or check on the live deploy.
4. **Images are `unoptimized:true`** (no runtime `sharp`; identical on every host). So **pre-compress heavy assets at the source** — e.g. the hero globe is `public/images/hero/globe.webp` (202 KB, was a 1.9 MB PNG). Don't reintroduce large raw PNGs into `/public` for above-the-fold use; convert with `ffmpeg -i x.png -c:v libwebp -quality 90 x.webp` (PIL here lacks WebP; `npm i` is currently broken under node 25/npm 11.9 → can't add `sharp`).
5. **Outstanding** (need client's eye or a new asset): exact globe pin spots, complete-globe-from-bottom framing on desktop, a higher-res (4K) globe image (current globe WebP is 1200²), and an animated/motion section-break wave system.
...
**Active workstream:** porting a Claude full-site visual redesign (bundle: `C:\Users\vsfag\Downloads\homepage_optmz_1\design_handoff_site\`) page-by-page — liquid-glass design system, display-Fraunces typography, logo-visor cascade, boarding-pass/departures motifs. **Done & verified live:** globals.css foundation, the collapsing blue-glass nav, the hero ("Wake up in <city>"), and now the **ENTIRE HOMEPAGE** — every section ported & verified by rendering `:3000` vs the design `:8766` (incl. the two the prior agent got wrong: **How-it-works = sticky boarding-pass builder**, **Categories = sticky visor-goggle glide** — both live in `home-sections-c.jsx` which overrides `-a` in the prototype). Gotcha logged: `body{overflow-x:hidden}` breaks `position:sticky` → use `clip`. The redesign is now **FULLY PORTED — all 5 remaining pages done & verified** (rendered `:3000` vs `:8766`): **Destinations** (globe hero + rotating Spotlight + sticky region board + bento grid), **Packages** (spec cards: duration segment-bar + difficulty 3-dot meter + %OFF + featured-span; category board Leisure/Adventure + sort), **Package Detail** (cinematic hero + SVG altitude chart from real elevation + dark route map with numbered pins/arcs/legend + pinned day-by-day + sticky booking bar; keeps vehicle-pricing table + visa forms + inclusions), **Visa** (all-navy passport-spread checklist w/ real fees/docs + ink stamp), **Vehicles** (fleet spec plates w/ parametric silhouettes + PAX gauges + gold Tempo Traveller). Ported by parallel sub-agents on a strict no-simplification brief, then visually verified per page. **Deploy status:** Homepage redesign + Education removal are LIVE (commit 0cfc4af, travelsense.co.in). The 5 page ports are the next deploy (validated via `next build`). Globe, real copy, data, and routes kept intact throughout.
