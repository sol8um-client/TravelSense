# -*- coding: utf-8 -*-
"""
Purge WRONG landmark images (people/abstract pages that the search wrongly
matched) by keeping an image ONLY if its Wikipedia page is a real geographic
PLACE (has coordinates). Also upgrades kept images to original resolution
(fixes low-quality). Run reconcile after.
"""
...
def api(q,ctry):
    url=("https://en.wikipedia.org/w/api.php?action=query&format=json&redirects=1"
         ...
         "&prop=coordinates|pageimages&piprop=original|thumbnail&pithumbsize=1400")
    ...
    for _,p in ((j.get("query") or {}).get("pages") or {}).items():
        has_coord="coordinates" in p
        src=(p.get("original") or p.get("thumbnail") or {}).get("source")
        return has_coord, src
    return False,None

kept=purged=miss=0
for n,(q,ctry) in enumerate(want.items(),1):
    slug=slugify(q); out=os.path.join(LMDIR,slug+".webp")
    try:
        has_coord,src=api(q,ctry)
        if has_coord and src and re.search(r"\.(jpg|jpeg|png)$",src,re.I):
            ext=".png" if src.lower().endswith(".png") else ".jpg"
            tmp=os.path.join(LMDIR,"_c_"+slug+ext)
            ...
            r=subprocess.run(["ffmpeg","-hide_banner","-loglevel","error","-y","-i",tmp,
                "-vf","scale='min(1400,iw)':-2","-c:v","libwebp","-quality","82",out],capture_output=True,text=True)
            os.remove(tmp)
            if r.returncode==0: kept+=1
            else: miss+=1
        else:
            if os.path.exists(out): os.remove(out); purged+=1
            else: miss+=1
    except Exception:
        miss+=1
    time.sleep(0.3)
    if n%150==0: print(f"  {n}/{len(want)} kept={kept} purged={purged}",flush=True)
print(f"DONE coord-filter: kept(place)={kept} purged(non-place)={purged} miss/none={miss}")
